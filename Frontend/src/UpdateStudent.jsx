...
function UpdateStudent() {
    ...
    const updateStudentData =(e) =>{
      e.preventDefault()
      axios.put('http://localhost:8000/student/update/'+id, {name, gender, address, contact, email, faculty})
      .then((result)=>{
        console.log(result)
        navigate('/Students')
      })
  ...
  return (
    <div className="main-body-wrapper w-5/6 bg-slate-200 float-right p-[10px] h-[95vh]">
      <div className="main-body p-[10px] flex flex-col w-full h-auto">
        <h2 className="uppercase text-[#25233f] font-bold text-[24px]">Update Students</h2>
        
                <div className='p-5 mx-5 my-2 formContainer'>
            
                <form className="p-7 bg-white w-[800px] mx-auto my-0 rounded-lg shadow-xl" onSubmit={updateStudentData}>
                ...
                <div className="w-full px-3">
                  ...
                  <input className="block w-full px-4 py-3 text-xl leading-tight text-gray-700 bg-gray-200 border border-gray-200 rounded appearance-none focus:outline-none focus:bg-white focus:border-gray-500" id="grid-faculty" type="text" value={faculty} onChange={(e) => setFaculty(e.target.value)}/>

                  <div className='flex justify-center mt-6'>
                  <button className="px-4 py-2 font-bold text-white bg-[#5D76A9] hover:bg-[#4C516D] rounded shadow focus:shadow-outline focus:outline-none" type="submit">
                    Update
