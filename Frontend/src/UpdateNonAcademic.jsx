...
function UpdateNonAcademic() {
      ...
      useEffect(() =>{
        axios.get('http://localhost:8000/NonAcademic/'+id)
        .then((result) =>{
      ...
      const updateStaffData= ((e) =>{
        e.preventDefault();
        axios.put('http://localhost:8000/NonAcademic/update/'+id, {name, gender, address, contact, email, position})
        .then((result) =>{
          console.log(result)
  ...
  return (
    <div className="main-body-wrapper w-5/6 bg-slate-200 float-right p-[10px] h-[95vh]">
      <div className="main-body p-[10px] flex flex-col w-full h-auto">
        <h2 className="uppercase text-[#25233f] font-bold text-[24px]">Update Non-Academic Staff</h2>
        
                <div className='p-5 mx-5 my-2 formContainer'>
            
                <form className="p-7 bg-white w-[800px] mx-auto my-0 rounded-lg shadow-xl" onSubmit={updateStaffData}>
...
export default UpdateNonAcademic